...
const Dijkstra: React.FC = () => {
    ...
    const validateGraph = useCallback((): string | null => {
        ...
        if (edges.some(e => parseFloat(e.weight || '1') < 0)) return "Dijkstra no admite aristas con peso negativo.";
        return null;
    }, [nodes, edges]);
    ...
    const runSimulation = useCallback(async () => {
        ...
        // Use selected node or first node as start
        let startNodeId = selectedNode;
        if (!startNodeId) {
        ...
        // Simple backtrace to find all critical edges that make up the shortest paths
        const backtrace = (node: string) => {
            critNodes.add(node);
        ...
        nodes.forEach(n => {
            if (dist[n.id] !== Infinity && n.id !== startNodeId) {
                backtrace(n.id);
            }
        });
        ...
        const dfs = (curr: string, currentPath: string[]) => {
            ...
            if (!pList || pList.length === 0) return;
            for (const p of pList) {
                dfs(p, [curr, ...currentPath]);
            }
        };
        
        // Find leaf nodes in shortest path tree
        const getOutgoingCritEdges = (nodeId: string) => edges.filter(e => e.source === nodeId && critEdges.has(e.id));
        nodes.forEach(n => {
            if (dist[n.id] !== Infinity && n.id !== startNodeId && getOutgoingCritEdges(n.id).length === 0) {
                dfs(n.id, []);
            }
        });
        ...
        setSimState('done');
    }, [nodes, edges, validateGraph, selectedNode]);
    ...
    const handleNodeClick = (e: React.MouseEvent, nodeId: string) => {
        ...
        if (selectedNode === nodeId) {
            message.error("No se permiten bucles (self-loops). Dijkstra no es compatible con pesos negativos.");
            setSelectedNode(null);
            return;
        }
    ...
    const handleMatrixEdgeChange = (sourceId: string, targetId: string, value: string) => {
        ...
        setEdges(prev => {
            ...
            if (num < 0) {
                message.error("El Algoritmo de Dijkstra no permite aristas con peso negativo. Ingresa un valor ≥ 0.");
                return prev;
            }
                ...
                <CanvasOuter>
                    ...
                    {isSimActive && (
                        <div style={{ padding: '8px 16px', borderTop: '1px solid #f0f0f0', display: 'flex', gap: 16, fontSize: '0.75rem' }}>
                            <span style={{ color: '#92400e', fontWeight: 600 }}><span style={{ background: '#fef3c7', borderRadius: 4, padding: '1px 6px', marginRight: 4 }}>Dist</span> Distancia Mínima</span>
                            <span style={{ color: '#ef4444', fontWeight: 600 }}>━ Caminos Más Cortos</span>
                        </div>
                    )}
                </CanvasOuter>
            ...
            {simState === 'done' && criticalPath.length > 0 && (
                <SummaryPanel>
                    <div style={{ fontWeight: 700, color: '#ef4444', marginBottom: 8, fontSize: '1rem' }}>
                        🔴 {criticalPath.length > 1 ? 'Caminos Más Cortos' : 'Camino Más Corto'} (Distancia total: {distances[criticalPath[0][criticalPath[0].length - 1]] ?? '?'} unidades)
                    </div>
                                        ...
                                        <React.Fragment key={`${pIdx}-${nodeId}`}>
                                            <span translate="no" style={{
                                                ...
                                                <span translate="no" style={{ fontSize: '0.7rem', opacity: 0.9 }}>
                                                    {' '}(Dist: {distances[nodeId] === Infinity ? '∞' : distances[nodeId]})
                                                </span>
                                            </span>
                    ...
                    <div style={{ marginTop: 10, fontSize: '0.78rem', color: '#64748b' }}>
                        Estas aristas forman las rutas más eficientes desde el nodo de inicio.
                    </div>
                </SummaryPanel>
            )}
