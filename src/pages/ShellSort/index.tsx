...
import { Button, Input, InputNumber, Slider, Radio, message } from 'antd';
import {
...
const ShellSort: React.FC = () => {
  ...
  const handleLoadManualArray = useCallback(() => {
    if (simPhase === 'running') return;

    const rawItems = manualText
      .split(',')
      .map(v => v.trim())
      .filter(v => v !== '');

    if (rawItems.length === 0) {
      message.warning('Ingresa un arreglo separado por comas');
      return;
    }

    const numericArray = rawItems.map(v => Number(v));

    if (numericArray.some(v => Number.isNaN(v))) {
      message.error('Todos los valores deben ser números válidos');
      return;
    }

    if (numericArray.length > MAX_ELEMS) {
      message.warning(`Máximo ${MAX_ELEMS} elementos. Se tomarán los primeros ${MAX_ELEMS}.`);
    }

    stopRef.current = true;
    setElements(makeElements(numericArray.slice(0, MAX_ELEMS)));
    setManualText('');
    setSimPhase('idle');
    setComparisons(0);
    setSwaps(0);
  ...
  const handleExportJSON = useCallback(() => {
    ...
    const rawName = window.prompt('Nombre del archivo', 'shell-sort-data');

    if (rawName === null) return;

    const cleanName = rawName.trim().replace(/[\\/:*?"<>|]+/g, '_');
    const finalName = cleanName === '' ? 'shell-sort-data' : cleanName;
    ...
    message.success(`Archivo exportado como ${finalName}.json`);
  }, [elements]);
  ...
  const handleSolve = useCallback(async () => {
    ...
    const shouldShift = (leftValue: number, rightValue: number) => {
    ...
    const markDone = async () => {
      for (let idx = 0; idx < n; idx++) {
        ...
        show(arr.map((_, i) => (i <= idx ? 'sorted' : 'default')));
        await sleep(Math.max(90, speedRef.current * 0.25));
      }
    };
    ...
    for (let gap = Math.floor(n / 2); gap > 0; gap = Math.floor(gap / 2)) {
      ...
      for (let i = gap; i < n; i++) {
        ...
        while (j >= gap) {
          ...
          show(arr.map((_, idx) => {
            if (idx === j) return 'outer';
            if (idx === j - gap) return 'scanning';
            return 'default';
          }));
          await sleep(speedRef.current);

          if (shouldShift(arr[j - gap], temp)) {
            arr[j] = arr[j - gap];
            movements++;
            setSwaps(movements);

            show(arr.map((_, idx) => {
              if (idx === j || idx === j - gap) return 'swapping';
              return 'default';
            }));
  ...
  return (
    <Wrap>
      <TopBar>
        <div>
          <PageTitle>Shell Sort</PageTitle>
          <div style={{ fontSize: '0.8rem', color: '#6b7280', marginTop: 2 }}>
            Algoritmo de ordenamiento por incrementos
          </div>
        </div>
      </TopBar>
      ...
      <MainArea>
        <ControlPanel>
          ...
          <Card>
            ...
            <FieldRow>
              <FieldLabel>Arreglo separado por comas</FieldLabel>
              <Input
                value={manualText}
                onChange={e => setManualText(e.target.value)}
                onPressEnter={handleLoadManualArray}
                disabled={isRunning}
                placeholder="Ej: 8, 3, 1, 9, 4"
                style={{ borderRadius: 8 }}
              />
            </FieldRow>
            <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
              <Button
                icon={<PlusOutlined />} onClick={handleLoadManualArray}
            ...
            <Button
              danger block icon={<DeleteOutlined />} onClick={handleClear}
              disabled={isRunning || elements.length === 0}
          ...
          {(isRunning || isPaused || isDone) && (
            <StatsRow>
              ...
              <StatBadge style={{ color: '#16a34a' }}>
                🔄 Movimientos: <strong style={{ marginLeft: 4 }}>{swaps}</strong>
              </StatBadge>
              {isDone && (
                <StatBadge style={{ color: '#16a34a' }}>
          ...
          <CanvasCard ref={canvasRef}>
            ...
            {elements.length === 0 ? (
              <div style={{
                ...
                Genera elementos aleatorios o carga un arreglo manual separado por comas
              </div>
            ) : (
              <BarsContainer>
                {elements.map((el, i) => {
