...
const BarRect = styled.div<BarRectProps>`
  ...
  cursor: default;
  user-select: none;
  /* GPU compositing layer – prevents repaints from bleeding into siblings */
...
const TriangleUp = styled.div<{ visible: boolean; triColor: string; entering: boolean }>`
  ...
  margin-top: 5px;
  /* Smooth color change without restarting animation */
          ...
          {(isRunning || isPaused || isDone) && (
            <StatsRow>
              ...
              <StatBadge style={{ color: '#16a34a' }}>
                🔄 Intercambios: <strong style={{ marginLeft: 4 }}>{swaps}</strong>
              </StatBadge>
              {isDone && (
                <StatBadge style={{ color: '#16a34a' }}>
